            ...
            if (!isStatic) {
                ...
                // determine ratios
                 for (int x = 0; x < data.length; x++) {
                    for (int y = 0; y < data[0].length; y++) {
                        ...
                        } else if (Math.abs(data[x][y].getBinValue() - data[x][y].getOriginalValue()) < low) {
                            low = Math.abs(data[x][y].getBinValue() - data[x][y].getOriginalValue());
                        }
                    }
                 }
                ...
                for (int x = 0; x < data.length; x++) {
                    for (int y = 0; y < data[0].length; y++) {
                        ...
                        double scale;
                        if (high == 0) scale = 0;
                        else scale = (double)cellDifference / ((double)high - (double)low);
                    ...
                    if (currentLine.hasMoreTokens()) {
                        ...
                        try {
                            if (!data[x][y].getText().equalsIgnoreCase(currentToken)) {
                                ...
                                System.out.println(data[x][y].getCompareValue());
                            }
                        } catch (ArrayIndexOutOfBoundsException ex) { /* copied table is larger than current table*/ }
                    }
                }             

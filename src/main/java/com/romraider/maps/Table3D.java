    ...
    public void verticalInterpolate() {
        ...
        if (coords[3] - coords[1] > 1) {
            ...
            for (y = coords[0]; y <= coords[2]; y++) {
            	diff = (tableData[y][coords[1]].getValue() - tableData[y][coords[3]].getValue()) / (coords[3] - coords[1]);
            	if (Math.abs(diff) > 0) {
	            	for (x = coords[1] + 1; x < coords[3]; x++)
	            		tableData[y][x].setRealValue(String.valueOf(tableData[y][x - 1].getValue() - diff));
            	}
            }
        }
    }
    ...
    public void horizontalInterpolate() {
        ...
        if (coords[2] - coords[0] > 1) {
            ...
            for (x = coords[1]; x <= coords[3]; x++) {
            	diff = (tableData[coords[0]][x].getValue() - tableData[coords[2]][x].getValue()) / (coords[2] - coords[0]);
            	if (Math.abs(diff) > 0) {
	            	for (y = coords[0] + 1; y < coords[2]; y++)
	            		tableData[y][x].setRealValue(String.valueOf(tableData[y - 1][x].getValue() - diff));
            	}
            }
        }
    }

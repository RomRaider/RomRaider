...
public class Table3D extends Table {
    ...
    public void compare(Table compareTable, int compareType) {
        ...
        if(!xAxis.isStatic) {
            int i = 0;
            for (DataCell cell : compareTable3D.xAxis.getData()) {
                xAxis.getData()[i].setCompareValue(cell.getOriginalValue());
                i++;
            }
            xAxis.compare(compareTable3D.xAxis, compareType);
        }

        if(!yAxis.isStatic) {
            int i = 0;
            for (DataCell cell : compareTable3D.yAxis.getData()) {
                yAxis.getData()[i].setCompareValue(cell.getOriginalValue());
                i++;
            }
            yAxis.compare(compareTable3D.yAxis, compareType);
        }
    }

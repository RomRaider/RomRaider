...
public class CompareImagesForm extends JFrame implements ActionListener {
	...
	private JComboBox<Rom> comboBoxImageLeft;
	private JComboBox<Rom> comboBoxImageRight;
    private JButton btnCompare;
    private JList<ListItem> listChanges;
	private DefaultListModel<ListItem> listModelChanges = new DefaultListModel<ListItem>();
	...
	public void compareTables(Rom left, Rom right)
	{
		...
		if(equal > 0 && different == 0 && missing == 0) {
			...
			lblImageResultString.setForeground(this.equal);
		} else if(different > 0) {
			lblImageResultString.setText("Images are NOT equal.  Equal Tables: "+equal+", Changed Tables: "+different+", Missing Tables: "+missing);
			lblImageResultString.setForeground(this.different);
		} else {
			lblImageResultString.setText("Images are NOT equal.  Equal Tables: "+equal+", Changed Tables: "+different+", Missing Tables: "+missing);
			lblImageResultString.setForeground(this.missing);
		}		
